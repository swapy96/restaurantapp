    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setIsLoading(false);
          setMsg("Image upladed successfully.");
      ...
      <div className="w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div className="w-full flex flex-col  md:flex-row items-center gap-3">
          <div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
            ...
            <input type="text" required placeholder="How many Calories?" value={calories} onChange={(e) => setCalories(e.target.value)} className="w-full h-full text-lg bg-transparent outline-none border-none placeholder:text-gray-400 text-textColor" />
          </div>
        ...
        <div className="flex items-center w-full">
          <button type="button" className="ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 px-12 py-2 rounded-lg text-lg text-white font-semibold" onClick={saveFoodItem}>
            SAVE
          </button>
        </div>
      </div>
